...
private:
    ...
    void performMeasurementUpdates()
    {
        ...
        constexpr size_t K_NEAREST = 3; // Use 3-nearest neighbors for data association
        ...
        for (size_t i = start_idx; i < cone_polar_positions_.size(); ++i)
        {
            ...
            // Find the K_NEAREST unused global cones (store idx, dist^2)
            std::vector<std::pair<size_t, double>> nearest;
            ...
            // Only consider K_NEAREST closest, and only if under threshold
            size_t num_associated = 0;
            for (size_t k = 0; k < std::min(K_NEAREST, nearest.size()); ++k)
            {
                size_t best_idx = nearest[k].first;
                double min_dist = nearest[k].second;
                if (min_dist < 2.0) // threshold for association in meters^2
                {
                    if (used_global_indices.count(best_idx) == 0)
                    {
                        used_global_indices.insert(best_idx);
                        measurementUpdate(z, cone_positions_[best_idx]);
                        ++num_associated;
                        RCLCPP_INFO(this->get_logger(),
                                    "Associated local cone (r=%.2f, th=%.2f) "
                                    "to global cone [%.2f, %.2f] (dist=%.2f)",
                                    z(0), z(1),
                                    cone_positions_[best_idx].x(),
                                    cone_positions_[best_idx].y(),
                                    std::sqrt(min_dist));
                    }
                }
                // Only use up to K_NEAREST matches per local cone
                if (num_associated >= K_NEAREST)
                    break;
            }
            if (num_associated == 0)
            {
